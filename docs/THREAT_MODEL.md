...
| Replay | Old approval is reused for a different payload | Decision IDs bind approval to one decision object; resolved decisions cannot be resolved again | Single-use decision tests |
...
4. **Tool boundary** — typed tools and connectors perform effects only under policy and identity controls.
